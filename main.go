...
func getRating(maxNodes int, level int) int {
	lowerelo := 800+((level-1)*75)
	lowernodes := getNodes(level)
	uppernodes := getNodes(level + 1)
	if maxNodes != 0 {
		return (lowerelo + ((75 * (maxNodes - lowernodes)) / (uppernodes - lowernodes)))
	} else {
		return lowerelo
	}
}
...
func sortMatches(matches []Match) {
    sort.Slice(matches, func(i, j int) bool {
        ...
        if maxI != maxJ {
            return maxI > maxJ // 3050 vs 1400 > 3000 vs 2000
        }
        ...
        // Tertiair: som van punten (oude gedrag)
...
func main() {
    ...
    for {
        ...
        case "1":
            ...
            sortMatches(lastMatches) // ← NIEUW: sorteer vóór wegschrijven
